...
# Set variables from user data arguments
PROJECT_NAME="vultr"
PROJECT_DOMAIN="vultr.solotest.win"
...
if [ -z "$PROJECT_NAME" ] || [ -z "$PROJECT_DOMAIN" ]; then
    echo "Error: PROJECT_NAME and PROJECT_DOMAIN must be provided as arguments."
    exit 1
fi
...
# --- Common Installation Steps (if any) ---
# (Currently, no common steps before OS-specific ones)
...
if [[ "$OS_ID" == "amzn" ]]; then
    ...
    sudo dnf install -y mariadb105 # Original package name for Amazon Linux
    if [ $? -ne 0 ]; then
    ...
    # Install MariaDB client
    echo "Installing MariaDB client..."
    sudo dnf install -y mariadb # Updated package name for CentOS 9+
    if [ $? -ne 0 ]; then
...
# --- WordPress Download and Setup (Common for all OS) ---
...
# --- Apache Virtual Host Configuration (OS-Specific nuances handled by variables) ---
